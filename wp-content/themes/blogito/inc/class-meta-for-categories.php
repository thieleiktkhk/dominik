	...
	class Meta_For_Categories {
		...
		public function new_term_bg_color_field() {
			...
			<div class="form-field mfc-term-bg-color-wrap">
				<label for="mfc-term-bg-color"><?php esc_html_e( 'Background Color', 'blogito' ); ?></label>
		...
		public function new_term_text_color_field() {
			...
			<div class="form-field mfc-term-text-color-wrap">
				<label for="mfc-term-text-color"><?php esc_html_e( 'Text Color', 'blogito' ); ?></label>
		...
		public function edit_term_bg_color_field( $term ) {
			...
			<tr class="form-field mfc-term-bg-color-wrap">
				<th scope="row"><label for="mfc-term-bg-color"><?php esc_html_e( 'Background Color', 'blogito' ); ?></label></th>
				<td>
		...
		public function edit_term_text_color_field( $term ) {
			...
			<tr class="form-field mfc-term-text-color-wrap">
				<th scope="row"><label for="mfc-term-text-color"><?php esc_html_e( 'Text Color', 'blogito' ); ?></label></th>
				<td>
		...
		public function new_term_image_field( $term ) {
			...
			<div class="form-field mfc-category-form-field">
				<label for="mfc_category_image_imageholder"><?php esc_html_e( 'Image', 'blogito' ); ?></label>
				...
				<button class="button" id="mfc_category_image_upload_button"><?php esc_html_e( 'Upload', 'blogito' ); ?></button>
				<button class="button" id="mfc_category_image_remove_button"><?php esc_html_e( 'Remove', 'blogito' ); ?></button>
				</div>
			</div>
		...
		public function edit_term_image_field( $term ) {
			...
			<tr class="form-field mfc-category-form-field">
				...
				<th scope="row">
				<label for="taxonomy_image"><?php esc_html_e( 'Category Image', 'blogito' ); ?></label>
				</th>
		...
		public function save_term_bg_color( $term_id ) {
			...
			if ( $old_color && '' === $new_color ) {
				delete_term_meta( $term_id, 'bg_color' );
		...
		public function admin_enqueue_scripts( $hook_suffix ) {
			...
			wp_enqueue_script(
				'category-image-js', get_template_directory_uri() . '/inc/js/categoryimage.js', array( 'jquery', 'wp-color-picker' ), '1.0.0', true
			);

			$data = array(
				// 'wp_version' => WP_VERSION,
				'label' => array(
					'title' => esc_html__( 'Choose Category Image', 'blogito' ),
					'button' => esc_html__( 'Choose Image', 'blogito' ),
				),
			);

			wp_localize_script(
				'category-image-js', 'CategoryImage', $data
			);
